...
column_names = ['Time (UTC)',
                ...
                'Dome door status', # closed: 0 | open: 4
                'Rain sensor (1)?',
                'Another temperture sensor?',
                'Sky-ambient',
                'Sky-ambient uncertainty',
                'Some loud/rain sensor?',
                'Rain sensor (2)?',
                'Rain sensor (3)?',
                'Rain since 9am',
                'SQM sky brightness']
...
# -----------------------------------------------------------------------------
# AAG Cloud Sensor Class    +++ AAT weather data
# -----------------------------------------------------------------------------
class AAGCloudSensor(object):
    ...
    * The following reading cycle takes just less than 3 seconds to perform:
    ...
    # --------------------------------------------------------------------------
    # obtaining and checking AAT weather data
    # --------------------------------------------------------------------------
    def fetch_AAT_met_data(self):
        # Download met data file
        m = requests.get("http://site.aao.gov.au/met/metdata.dat")
        ...
        t = Table.read(met, format='ascii.no_header', delimiter='\t', names=column_names)
        ...
        t['Outside air temperature'].unit = u.Celsius
        t['Dome air temperature'].unit = u.Celsius
        t['Mirror temperature'].unit = u.Celsius
        t['Outside dewpoint'].unit = u.Celsius
        t['Outside humidity'].unit = u.percent
        t['Barometric pressure'].unit = cds.mmHg
        t['Average wind speed'].unit = u.km / u.hour
        t['Maximum wind gust'].unit = u.km / u.hour
        t['Average wind direction'].unit = u.degree
        t['Sky-ambient'].unit = u.Celsius
        t['Rain since 9am'].unit = u.mm
        t['SQM sky brightness'].unit = u.ABmag

        return t

    # using sky-ambient to get cloud conditions
    def _get_AAT_cloud_safety(self):
            safety_delay = self.safety_delay

            entries = self.fetch_AAT_met_data()
            threshold_cloudy = self.cfg.get('threshold_cloudy', -22.5)
            threshold_very_cloudy = self.cfg.get('threshold_very_cloudy', -15.)

            sky_amb = entries['Sky-ambient']
            sky_amb_u = entries['Sky-ambient uncertainty']

            if len(sky_amb) == 0:
                print('AAT UNSAFE: no sky temperatures found')
                sky_safe = False
                cloud_condition = 'Unknown'
            else:
                if (sky_amb + sky_amb_u) > threshold_cloudy:
                    self.logger.debug('AAT UNSAFE: Cloudy in last {:.0f} min. Max sky amb {:.1f} C'.format(
                    ...
                    if sky_amb > threshold_very_cloudy:
                        ...
                        cloud_condition = 'Clear'
                        self.logger.debug('AAT Cloud Condition: {} (Sky-Amb={:.1f} C)'.format(cloud_condition, sky_amb))

            return cloud_condition, sky_safe

    # checks wind safety conditions for the AAT data
    def _get_AAT_wind_safety(self):
            safety_delay = self.safety_delay
            entries = self.fetch_AAT_met_data()
            ...
            # Wind (average and gusts)
            wind_speed = entries['Average wind speed']
            wind_gust = entries['Maximum wind gust']

            if len(wind_gust) == 0:
                self.logger.debug('AAT UNSAFE: no maximum wind gust readings found')
                gust_safe = False
                gust_condition = 'Unknown'

            if len(wind_speed) == 0:
                self.logger.debug('AAT UNSAFE: no average wind speed readings found')
                wind_safe = False
                wind_condition = 'Unknown'
            else:
                # Windy?
                if wind_speed > threshold_very_windy:
                    self.logger.debug('AAT UNSAFE:  Very windy in last {:.0f} min. Average wind speed {:.1f} kph'.format(
                ...
                if wind_speed > threshold_very_windy:
                    ...
                    wind_condition = 'Calm'
                    self.logger.debug('AAT Wind Condition: {} ({:.1f} km/h)'.format(wind_condition, wind_speed))

                # Gusty?
                if wind_gust > threshold_very_gusty:
                    self.logger.debug('AAT UNSAFE:  Very gusty in last {:.0f} min. Max gust speed {:.1f} kph'.format(
                ...
                self.logger.debug('AAT Gust Condition: {} ({:.1f} km/h)'.format(gust_condition, wind_gust))

            return (wind_condition, wind_safe), (gust_condition, gust_safe)

    # ??? not too sure which of the three rain sensor items to use
    def _get_AAT_rain_safety(self, current_values):
        safety_delay = self.safety_delay
        entries = self.fetch_AAT_met_data()
        threshold_wet = self.cfg.get('threshold_wet', 2000.)
        threshold_rain = self.cfg.get('threshold_rainy', 1700.)
        ...
        if len(rf_value) == 0:
            ...
            if rain_safe:
                if min(rf_value) <= threshold_rain:
                    self.logger.debug('AAT UNSAFE:  Rain in last {:.0f} min.'.format(safety_delay))
                    rain_safe = False
                elif min(rf_value) <= threshold_wet:
                    self.logger.debug('AAT UNSAFE:  Wet in last {:.0f} min.'.format(safety_delay))
                    rain_safe = False
                else:
                    rain_safe = True

            self.logger.debug('AAT Rain Condition: {}'.format(rain_condition))
